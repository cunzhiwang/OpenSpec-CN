...
export class ValidateCommand {
  ...
  private printNonInteractiveHint(): void {
    console.error('没有要验证的内容。请尝试以下命令:');
  ...
  private async validateDirectItem(itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    ...
    if (!type) {
      ...
      if (suggestions.length) console.error(`您是否想输入: ${suggestions.join(', ')}?`);
      process.exitCode = 1;
      return;
    }

    if (!opts.typeOverride && isChange && isSpec) {
      console.error(`项目 '${itemName}' 同时匹配变更和规范，存在歧义。`);
  ...
  private printNextSteps(type: ItemType): void {
    ...
    if (type === 'change') {
      bullets.push('- 确保变更在 specs/ 中有增量: 使用标题 ## ADDED/MODIFIED/REMOVED/RENAMED Requirements');
      bullets.push('- 每个需求必须包含至少一个 #### Scenario: 块');
      bullets.push('- 调试已解析的增量: openspec change show <id> --json --deltas-only');
    } else {
